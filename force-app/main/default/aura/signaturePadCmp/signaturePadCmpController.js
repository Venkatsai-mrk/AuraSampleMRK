({
    doInit : function(component, event, helper) {
        var action=component.get("c.getUserCommunity");
        action.setCallback(this,function(response){
            console.log('get1' , response.getState());
            if (response.getState() == "SUCCESS"){
                console.log('getState' , response.getState());
                var userCommunityWrapper = response.getReturnValue();
                //if(userCommunityWrapper.isPortalEnabled == true){
                 //if(userCommunityWrapper.username == true){                    
                    //component.set("v.isPortalEnabled", userCommunityWrapper.isPortalEnabled);
                    component.set("v.username", userCommunityWrapper.username);
                    component.set("v.recordVal", userCommunityWrapper.AccountId);
                    console.log('getReturnValue', response.getReturnValue());
                    helper.handleClickForOTP(component, event, helper);                  
                //}
            }
        });       
        $A.enqueueAction(action); // added by bharti   
    ...
    save:function(component, event, helper){
        ...
        console.log('strDataURIdecode:='+strDataURI.length);
        if(strDataURI.length > 1592){
        ...
        $A.enqueueAction(action);
            ...
            toastEvent.setParams({
                ...
                "message": "Please add your signature to proceed."
            });
            toastEvent.fire();
        }
    },
    ...
    validateOTP:function(component,event,helper){
       ...
       var attVal = component.get("v.OTPVal") ;
        ...
        var action1 = component.get("c.matchOtp");
            console.log('OTPValOTPVal', component.get("v.OTPVal"));
            console.log('recordValrecordVal', component.get("v.recordVal"));
        action1.setParams({  
