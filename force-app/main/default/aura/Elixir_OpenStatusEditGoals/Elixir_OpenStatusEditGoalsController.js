({
    ...
    goalSection  : function(component, event, helper) {
       ...
       /* if($A.util.isUndefined(goal[nameSpace+'Dataset2__c'])){
        ...
        if(goalFirstCallBack == true){
            if(type == 'Template'){
                action = component.get("c.getTemplateTasks"); // Objective
            }else{
                action = component.get("c.getTasks"); // Objective
            }
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS") {   
                    ...
                    console.log('prop resp  '+JSON.stringify(response.getReturnValue()));
                    var goal = component.get("v.goal");
                    goal['listOfTask'] = listOfTasks;
                    goal['listOfIntervention'] = response.getReturnValue().listOfIntervention; // intervention
     ...
     addTask2 :	function(component, event, helper){
        ...
        var listOfTask = component.get("v.tasksList");
    ...
    dueDate  :		function(component, event, helper) {
        ...
        listOfGoals[goalIdx][nameSpace+'Due_Date__c'] = dueDate;
        if($A.util.isUndefinedOrNull(listOfGoals[goalIdx][nameSpace+'Dataset2__c'])){
            //No Action Taken
          }else{
            listOfGoals[goalIdx]['Action'] = 'UPDATE';;
          }
    ...
    //    console.log(JSON.stringify(component.get('v.goalsList')));
        ...
        if($A.util.isUndefinedOrNull(listOfGoals[goalIdx][nameSpace+'Dataset2__c'])){
           ...
           console.log('def');
            listOfGoals[goalIdx]['Action'] = 'DELETE';
      }
