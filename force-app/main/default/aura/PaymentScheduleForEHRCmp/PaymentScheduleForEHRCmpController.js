({
    ...
    //    var totalOut = component.get("v.patientOutstanding") ;
        ...
        {
            if(parseInt(installmentAmt1) > parseInt(totalPlanAmount))
            {
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "type" : "Error",
                    "title": "Problem Occured!",
                    "message": "Installment amount cannot be greater than Patient Responsibility amount."
                });
                toastEvent.fire();
            ...
            else
            {
                ...
                if(iteration !=0){
                    ...
                    var update=$A.localizationService.formatDate(dateForUse, "MM/DD/YYYY");
                    moneyResidue=(totalPlanAmount-Math.floor(iteration1)*installmentAmt).toFixed(2);
        ...
        console.log('list of values 2nd '+JSON.stringify(listOfFields));
