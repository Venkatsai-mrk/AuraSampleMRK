({
    doInit : function(component) {
        ...
        var pageReference = component.get("v.pageReference");
            ...
            var openApproval = pageReference.state.c__openApproval;
            if(openApproval != null && openApproval != ''){
                component.set("v.openApproval", openApproval);
            }
        ...
        action.setCallback(this , function(response){
            ...
            if(state == 'SUCCESS'){
                ...
                var initialProblemLength = respWrapper.listOfProblem.length;
               
                if(!$A.util.isUndefinedOrNull(respWrapper.listOfProblem)){
                    var listOfProblem = respWrapper.listOfProblem;
                    for(var i = listOfProblem.length - 1; i >= 0; i--) {
                        var problem = listOfProblem[i];
                        if ($A.util.isUndefinedOrNull(problem.ElixirSuite__Plan_Hierarchy__r)) {
                            listOfProblem.splice(i, 1);
                        }
                    }
                    respWrapper.listOfProblem = listOfProblem;
                }
                
                if(initialProblemLength > respWrapper.listOfProblem.length){
                    component.set("v.showProblemButton",true);
                }
    ...
    setClinicalSupervisor : function(component ,event){
        ...
        component.set("v.ElixirSuite__Clinical_supervisor__c" , value);
        component.set("v.carePlan" , obj);
    },
    closeMainModal  : function(component){
        ...
        console.log('contentDocumentId',component.get("v.contentDocumentId"));
        if(!$A.util.isEmpty(component.get("v.contentDocumentId"))){
            var action = component.get("c.delNotesOnCancel");
            action.setParams({ 
                noteId : component.get("v.contentDocumentId")
            });
            action.setCallback(this, function(response) {
                var state = response.getState();
                if (state === "SUCCESS") {
                    console.log('Call back delNotesOnCancel success');
    ...
    Save : function(component) {
        ...
        if(allValid==false || dateValid==false){
            ...
            // Add Goals from top if added newly during edit
            ...
            if(!$A.util.isUndefinedOrNull(component.get("v.allGoalsShow"))){
                ...
                masterProblems['listOfGoal'] = listOfTopLevelGoals;
            ...
            //Add Objective from top if added newly during edit
            if(!$A.util.isUndefinedOrNull(component.get("v.allTasksListShow"))){
                var listOfTasks = component.get('v.allTasksListShow');
                var problemsTask = component.get("v.masterProblems");
                problemsTask['listOfObjs'] = listOfTasks;
                component.set('v.masterProblems',problemsTask);
                console.log('component.intervent' +JSON.stringify(component.get('v.masterProblems')) );
            }
            
            //Add Intervention From top if added newly during edit
            if(!$A.util.isUndefinedOrNull(component.get("v.allIntervList"))){
                var allIntervList = component.get('v.allIntervList');
                var problems = component.get("v.masterProblems");
                problems['listOfTasks'] = allIntervList;
                component.set('v.masterProblems',problems);
            ...
            console.log('--> '+JSON.stringify(component.get("v.masterProblems")));
             console.log('approvedValues@@@'+JSON.stringify(component.get("v.approvedValues")));
            ...
            action.setParams({ 
                ...
                removedProblem: component.get("v.removedProblemId"),
                accountId : component.get("v.recordVal"),
                removedIntervention : component.get("v.removedInterventionId")
            });
            action.setCallback(this, function(response) {
                var state = response.getState();
                if (state === "SUCCESS") {   
