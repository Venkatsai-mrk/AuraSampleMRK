    ...
    updateRecord : function(component, event, helper) {
        try{
            if(component.get("v.recordDetail").ICDCodeLabel){
                ...
                console.log('recordVal to update '+JSON.stringify(component.get("v.recordVal")));
                console.log('RowId to update '+JSON.stringify(component.get("v.RowId")));
                var action = component.get("c.updateProblemRec");
                action.setParams({'payload' : JSON.stringify(payloadToSave),
                                  'accountId' : component.get("v.recordVal"),
                                  'recordInViewId' : component.get("v.RowId")});
                action.setCallback(this, function(response) {
                    var state = response.getState();
                    if (state === "SUCCESS") {                
                        component.set("v.loaded",true);
