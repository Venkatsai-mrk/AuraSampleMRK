({
    ...
    initHelperCallback : function(component, event, helper) {
        ...
        action.setParams({
            "formNameInPainText": component.get("v.formName")
        });        
        action.setCallback(this, function(response) { 
            ...
            if (state === "SUCCESS") {
                ...
                if(!$A.util.isEmpty(response.getReturnValue().numberOfApprovalLevels)){
                    var numberOfApprovalLevels = response.getReturnValue().numberOfApprovalLevels[0].ElixirSuite__Maximum_Level_For_Approval__c;
