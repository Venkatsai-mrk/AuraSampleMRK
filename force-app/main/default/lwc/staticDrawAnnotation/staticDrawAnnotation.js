...
//import staticResources from "@salesforce/resourceUrl/ClientImages";
...
import drawAnnotateMC from '@salesforce/messageChannel/drawAnnotateFormMC__c';
...
export default class DrawAnnotation extends LightningElement {
    ...
    error
    @api staticName = '';
    //resourcePath = staticResources + staticName + '.jpg;';
      ...
      savedImage({accountId: this.accountId, formUniqueId:this.formUniqueId, 
            ...
            .then(result => {
                ...
                if(result){
                    ...
                    publish(this.messageContext, drawAnnotateMC, payload);
                }
            })
            .catch(error => {
...
////////////////////////Confirmation prompt////////////////////////////
    const result= await LightningConfirm.open({
         message: 'This will completely clear out all modifications done on the canvas. Are you sure you want to select this option?',
        variant: 'headerless',
        ...
        if (result==true) {
        savedImage({accountId: this.accountId, formUniqueId:this.formUniqueId, 
         formCssId:this.formCssId, isNew:true, resourceName :this.staticName})
           .then(result => {
             console.log('result data '+result);
             if(result){
            this.getCanvas().currentCanvasValue = {"data":'image/png;base64,'+result};
